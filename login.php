...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($result->num_rows > 0) {
        ...
        if (password_verify($password, $user['password'])) {
            ...
            $_SESSION['display_username'] = $user['display_username'];
            $_SESSION['role'] = $user['role'];

            if ($user['is_first_login'] == 1) {
                header('Location: customize_profile.php');
            } else {
                header('Location: ' . ($user['role'] == 'Member' ? 'member_dashboard.php' : 'manager_dashboard.php'));
            }
            exit();
        } else {
            $alert = "<div class='alert alert-danger alert-dismissible fade show' role='alert'>
                        <strong>Oops!</strong> Password yang Anda masukkan salah.
                        <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                      </div>";
        }
    } else {
        $alert = "<div class='alert alert-warning alert-dismissible fade show' role='alert'>
                    <strong>Maaf!</strong> User tidak ditemukan. Silakan cek kembali data Anda.
                    <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
                  </div>";
    }
}
